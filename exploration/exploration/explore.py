...
class Explorer(Node):
    ...
    def save_first_map(self, map):
        grid = np.array(map.data).reshape((map.info.height, map.info.width))
        filename = os.path.join(os.path.expanduser('~'), 'maps', 'first_occupancy_grid.txt')
    ...
    def explore_frontier(self, map):
        ...
        if paths:
            ...
            self.get_logger().info('No path was found to the target')
