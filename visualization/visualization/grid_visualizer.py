...
class GridMapVisualizer(Node):
    ...
    def occupancy_callback(self, msg):
        ...
        prev_point = None  # Initialize the previous point

        for pose in self.robot_path.poses:
            # Convert the current pose position to scaled integer coordinates
            current_point = (int(pose.pose.position.x * 20), int(pose.pose.position.y * 20))

            # Draw a line if there is a previous point
